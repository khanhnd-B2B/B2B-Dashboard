...
class B2BTonAdvisor:
    ...
    def reply_route_detail(self, chat_id, thread_id, route_query):
        """Phản hồi chi tiết các điểm đi trong lộ trình xe khi người dùng yêu cầu."""
        try:
            msg = self.lookup_route(route_query)
            self.send_telegram(msg, chat_id=chat_id, thread_id=thread_id)
            BOT_STATUS['last_action'] = f"Tra cứu lộ trình '{route_query}' lúc {get_vietnam_now().strftime('%H:%M:%S')}"
        except Exception as e:
            err_msg = f"❌ <b>Lỗi tra cứu lộ trình:</b> <code>{str(e)}</code>"
            print(f"Lỗi lookup_route: {e}")
    ...
    def lookup_route(self, query):
        ...
        if self.df_truck is None or self.df_truck.empty:
            return "⚠️ Không có dữ liệu lịch tải xe hiện tại để tra cứu!"
    ...
    def _format_route_detail(self, matched_df):
        ...
        # Khung giờ xuất bến
        if 'HHMM' in matched_df.columns:
            times = sorted([str(t) for t in matched_df['HHMM'].dropna().unique() if str(t) != 'nan' and str(t).strip()])
        elif 'GioDuKienBatDau_GMT7' in matched_df.columns:
            dt_col = pd.to_datetime(matched_df['GioDuKienBatDau_GMT7'], errors='coerce')
            times = sorted([t for t in dt_col.dt.strftime('%H:%M').dropna().unique() if t])
        else:
            times = []
        time_str = ', '.join(times) if times else 'Theo điều phối'

        # Tải trọng xe
        trong_tai = 0
        if 'TrongTai' in matched_df.columns:
            tt_vals = [int(v) for v in pd.to_numeric(matched_df['TrongTai'], errors='coerce').dropna().unique() if int(v) > 0]
            if tt_vals:
                trong_tai = tt_vals[0]
        if trong_tai >= 1000:
            tt_str = f"{trong_tai:,} kg ({trong_tai / 1000:.1f} tấn)".replace('.0 tấn', ' tấn')
        elif trong_tai > 0:
            tt_str = f"{trong_tai:,} kg"
        else:
            tt_str = "Xe cố định / hợp đồng"

        # Danh sách điểm dừng
        stops = []
        if 'ToanBoDiemDi' in matched_df.columns:
            sub = matched_df[matched_df['ToanBoDiemDi'].notna() & (matched_df['ToanBoDiemDi'].astype(str).str.strip() != '')]
            if not sub.empty:
                tbdd = sub['ToanBoDiemDi'].iloc[-1]
                stops = [s.strip() for s in re.split(r'→|->|➔', str(tbdd)) if s.strip()]
        if not stops and 'ThuTuDiem' in matched_df.columns and 'TenDiem' in matched_df.columns:
            first_chuyen = matched_df['MaChuyen'].iloc[0]
            trip_sub = matched_df[matched_df['MaChuyen'] == first_chuyen].sort_values('ThuTuDiem')
            stops = [str(td).strip() for td in trip_sub['TenDiem'].dropna() if str(td).strip()]

        total_stops = len(stops)

        lines = []
        lines.append(f"🚚 <b>LỘ TRÌNH TUYẾN:</b> <code>{ma_tuyen}</code>\n")
        lines.append(f"⏰ <b>Khung giờ xuất bến:</b> <b>{time_str}</b>")
        lines.append(f"⚖️ <b>Tải trọng xe:</b> <b>{tt_str}</b>")
        lines.append(f"📍 <b>Tổng số điểm dừng:</b> <b>{total_stops} điểm</b>\n")
        lines.append("📋 <b>DANH SÁCH CÁC ĐIỂM ĐI TRONG LỘ TRÌNH:</b>")

        if not stops:
            lines.append("<i>Chưa có dữ liệu danh sách điểm dừng chi tiết cho tuyến này.</i>")
        else:
            for idx, stop in enumerate(stops, 1):
                if idx == 1:
                    lines.append(f"<b>{idx}.</b> 🏁 <b>Điểm đầu (Xuất phát):</b> {stop}")
                elif idx == total_stops:
    ...
    def process_and_report(self, target_chat_id=None, target_thread_id=None, send_tele=True, window_hours=4):
        try:
            return self._process_and_report_internal(target_chat_id=target_chat_id, target_thread_id=target_thread_id, send_tele=send_tele, window_hours=window_hours)
    ...
    def _process_and_report_internal(self, target_chat_id=None, target_thread_id=None, send_tele=True, window_hours=4):
        now = get_vietnam_now()
    ...
    def register_commands(self):
        ...
        commands = [
            ...
            {'command': 'tuyen', 'description': 'Tra cứu lộ trình & điểm đi của tuyến xe (/tuyen <mã_tuyến>)'},
    ...
    def handle_telegram_update(self, update):
        ...
        # Case C: Tin nhắn gửi trực tiếp mã tuyến (ví dụ: HN_LaoCai_HN_03, hn_laocai_hn_03)
        route_tokens = re.findall(r'\b[A-Za-z0-9]+_[A-Za-z0-9]+(?:_[A-Za-z0-9]+)*\b', text)
        if route_tokens:
            if self.df_truck is None or getattr(self, '_last_truck_load_time', 0) < time.time() - 3600:
                self._load_truck_data()
                self._last_truck_load_time = time.time()
            if self.df_truck is not None and not self.df_truck.empty:
                known_routes = set(self.df_truck['MaTuyen'].dropna().astype(str).str.strip().str.upper())
                for tok in route_tokens:
                    if tok.upper() in known_routes:
                        self.reply_route_detail(chat_id, thread_id, tok)
                        return
            if len(route_tokens) == 1 and text.strip().upper() == route_tokens[0].upper():
                self.reply_route_detail(chat_id, thread_id, route_tokens[0])
                return
